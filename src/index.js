...
const [config, validConfig] = ((path) => {
    ...
    fs.existsSync(configPath)
        ? configPath
        : fs.existsSync(previousConfigPath)
        ? previousConfigPath
        : null
);
...
if (validConfig) {
    ...
    function generateShortcut(shortcut, index) {
        const element = blessed.text({
            ...
            content:
                "\n\n\n\n" +
                center(
                    typeof shortcut.name === "string" ? shortcut.name : "",
                    14
                ),
            tags: true,
            border: {
            ...
            style: {
                fg:
                    typeof shortcut.foreground === "string"
                        ? shortcut.foreground
                        : "black",
                bg:
                    typeof shortcut.background === "string"
                        ? shortcut.background
                        : "white",
                bold: true,
            },
    ...
    function selectRight() {
        selectionPos =
            displayedShortcuts.length <= selectionPos + 1
                ? 0
                : selectionPos + 1;
    ...
    screen.on("keypress", function (ch, key) {
        (() => {
            if ((key.name || key.full).length === 1 || key.full === "space") {
                content =
                    content.substring(0, caretPos) +
                    ch +
        ...
        (() => {
            if (key.full === "up" || (!isSearching && key.full === "left"))
                ...
                key.full === "down" ||
                (!isSearching && key.full === "right")
            )
                selectRight();
            else if (key.full === "enter" && displayedShortcuts.length !== 0) {
                const execute =
                    config.shortcuts[displayedShortcuts[selectionPos][1]]
                        .execute;
    ...
    message.content += center(
        "'~/.config/spotlighttui/config.json' has to be valid.",
        message.width
    );
    screen.render();
}
